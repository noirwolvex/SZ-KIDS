...
export default function App() {
    ...
    if (activeGame) {
      ...
      if (game) {
        try {
          ...
          if (coins > 0) showToast(`+${coins} coins earned!`, 'success', '🪙');
        } catch {
  ...
  const openLesson = useCallback((lessonId: string) => setActiveLesson(lessonId), []);
  ...
  if (platformHubOpen) return <BackgroundMusic><SpaceZoneHub onOpenPlatform={enterCurrentExperience} /></BackgroundMusic>;
...
function ConfettiBurst() {
  const pieces = Array.from({ length: 60 }).map((_, i) => ({ id: i, x: Math.random() * 100, delay: Math.random() * 0.3, duration: 2 + Math.random() * 1.5, rotate: Math.random() * 360, color: ['#ff7fbf', '#ffd24d', '#34c187', '#38bdf8', '#9d7ce6', '#ff8f63'][Math.floor(Math.random() * 6)], size: 8 + Math.random() * 10 }));
  return <div className="fixed inset-0 z-[200] pointer-events-none overflow-hidden">{pieces.map((p) => <motion.div key={p.id} className="absolute top-0" style={{ left: `${p.x}%`, width: p.size, height: p.size, backgroundColor: p.color, borderRadius: '2px' }} initial={{ y: -20, opacity: 1, rotate: 0 }} animate={{ y: '110vh', opacity: [1, 1, 0], rotate: p.rotate }} transition={{ duration: p.duration, delay: p.delay, ease: 'easeIn' }} />)}</div>;
}
