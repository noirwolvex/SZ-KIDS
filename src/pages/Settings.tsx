...
import {
  ...
  Languages, Mail, Lock, Info, Heart, Star, Smile, Clock, KeyRound,
} from 'lucide-react';
          ...
          <Section title="Screen Time & Parent PIN" subtitle="Limits and parental protection" icon={<Clock size={20} />} gradient="from-sky-200 to-mint-200">
            <div className="flex flex-col gap-2 py-3.5">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  <Clock size={18} className="text-lavender-400 shrink-0" />
                  <div>
                    <p className="text-sm font-display font-semibold text-lavender-500">Daily Screen Time</p>
                    <p className="text-xs text-lavender-400">Maximum minutes allowed per day</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <input
                    ...
                    max={240}
                    step={5}
                    value={form.screen_time_limit}
                    onChange={(e) => update('screen_time_limit', Math.min(240, Math.max(5, Number(e.target.value) || 5)))}
                    className="w-24 py-2.5 px-3 rounded-2xl bg-lavender-100 border border-lavender-200 text-lavender-500 font-display font-semibold text-center focus:outline-none focus:border-lavender-300"
            ...
            <div className="flex flex-col gap-2 py-3.5 border-t border-lavender-100">
              <div className="flex items-center gap-2 min-w-0">
                <KeyRound size={18} className="text-lavender-400 shrink-0" />
                <div>
                  <p className="text-sm font-display font-semibold text-lavender-500">Parent PIN</p>
                  <p className="text-xs text-lavender-400">Optional 4–6 digit PIN for parent-only controls</p>
                </div>
              </div>
              <input
                type="password"
                ...
                className="w-full py-3 px-4 rounded-2xl bg-white border-2 border-lavender-100 focus:border-lavender-300 focus:outline-none text-lavender-500 font-medium tracking-[0.35em]"
              ...
              {confirmSignOut ? (
                <motion.div key="confirm-signout" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                  <div className="py-3 space-y-3">
                    <p className="text-sm text-lavender-500 font-display font-semibold text-center">Are you sure you want to sign out?</p>
                    <div className="flex gap-2 justify-center">
                      <Button size="sm" variant="secondary" onClick={() => setConfirmSignOut(false)}>Cancel</Button>
                      <Button size="sm" variant="success" onClick={() => signOut()}><LogOut size={16} /> Sign Out</Button>
                    </div>
                  </div>
                </motion.div>
              ) : (
                <motion.div key="signout-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-3">
                  <button onClick={() => setConfirmSignOut(true)} className="w-full flex items-center justify-center gap-2 p-3.5 rounded-2xl bg-blush-50 hover:bg-blush-100 text-blush-500 font-display font-semibold text-sm transition-all active:scale-[0.98] touch-target-sm">
            ...
            <div className="mx-auto max-w-3xl px-4 pb-4">
              <div className="glass-strong rounded-3xl shadow-soft-lg p-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="relative shrink-0">
                    <motion.div animate={{ scale: [1, 1.15, 1] }} transition={{ duration: 2, repeat: Infinity }} className="w-9 h-9 rounded-xl bg-gradient-to-br from-lemon-200 to-peach-200 flex items-center justify-center">
                      <Sparkles size={18} className="text-peach-500" />
                    </motion.div>
                  </div>
                  <div className="min-w-0">
                    <p className="font-display font-semibold text-sm text-lavender-500 truncate">Unsaved changes</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <AnimatePresence>
                    {saved && (
                      <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} className="flex items-center gap-1.5 text-mint-500 font-display font-semibold text-sm">
                        <Check size={16} /> Saved!
                      </motion.div>
                    )}
                  </AnimatePresence>
                  <Button size="sm" onClick={handleSave} disabled={saving}>{saving ? <Loader2 size={16} className="animate-spin" /> : 'Save'}</Button>
                </div>
              </div>
            </div>
          </motion.div>
  ...
  return (
    <Card className="p-5 sm:p-6 mb-4">
      <div className="flex items-center gap-3 mb-3">
        <div className={`w-10 h-10 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center text-lavender-500 shrink-0`}>{icon}</div>
        <div><h2 className="font-display font-bold text-lavender-500">{title}</h2><p className="text-xs text-lavender-400">{subtitle}</p></div>
      </div>
      <div className="divide-y divide-lavender-100">{children}</div>
    </Card>
  );
}
...
function Toggle({ label, desc, value, onChange, icon }: { label: string; desc: string; value: boolean; onChange: (value: boolean) => void; icon: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between py-3.5 gap-3">
      <div className="flex items-center gap-2 min-w-0"><div className="shrink-0 w-8 h-8 rounded-xl bg-lavender-100 flex items-center justify-center text-lavender-400">{icon}</div><div><p className="text-sm font-display font-semibold text-lavender-500">{label}</p><p className="text-xs text-lavender-400">{desc}</p></div></div>
      <button type="button" role="switch" aria-checked={value} onClick={() => onChange(!value)} className={`relative shrink-0 w-12 h-7 rounded-full transition-colors ${value ? 'bg-mint-400' : 'bg-lavender-200'}`}>
        <span className={`absolute top-1 w-5 h-5 rounded-full bg-white shadow-soft transition-transform ${value ? 'left-6' : 'left-1'}`} />
      </button>
    </div>
  );
}
